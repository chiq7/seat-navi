...
export function SeatReportForm({
  ...
  const [internalEvent, setInternalEvent] = useState<CrawledEvent | null>(null);
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .maybeSingle()
      .then(({ data }) => {
        if (data) setInternalEvent(data as CrawledEvent);
      });
  }, [event, eventId]);

  const selectedEvent = event ?? internalEvent;
