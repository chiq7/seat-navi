...
function VenueCards({ venueIds, compact = false }: { venueIds: readonly string[]; compact?: boolean }) {
  return (
    <div className={`grid border-l border-t border-[#ded8dc] sm:grid-cols-2 ${compact ? "lg:grid-cols-4" : "lg:grid-cols-3"}`}>
      {venueIds.map((venueId, index) => {
        ...
        return (
          <Link
            ...
            className={`zr-focus group flex flex-col justify-between border-b border-r border-[#ded8dc] bg-white p-5 transition-colors hover:bg-[#fff0f5] ${compact ? "min-h-[130px]" : "min-h-[148px]"}`}
          >
  ...
  return (
    <main className="min-h-screen bg-[#f7f5f6] pb-16 text-[#1c171b]">
      ...
      <section className="zr-container py-10 sm:py-14" aria-labelledby="venue-list-title">
        ...
        <nav className="mt-7 grid grid-cols-2 border-l border-t border-[#ded8dc] text-[11px] font-black sm:flex" aria-label="会場タイプから探す">
          {[
            ["popular", "よく見られる"],
        ...
        <section id="popular" className="scroll-mt-5 mt-10 sm:mt-14" aria-labelledby="popular-title">
        ...
        <div className="mt-14 space-y-14 sm:mt-18 sm:space-y-18">
