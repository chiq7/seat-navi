...
- 公式NEWSのイベント候補65記事は、登録済み・既存・人手確認済み除外のいずれかに確定し、保留0件。試写会・映画館上映・展示・オンラインミーグリはNEWSだけを保持し、公演一覧には反映しない。
- 本番 `events` は510件。重複候補14組のうち真の重複10組・12行を整理済み。残る4組は別出演者・第一部/第二部等の別公演として保持する。
...
- 試写会・映画館上映・展示・オンラインミーグリは取得済み公式NEWSを再利用し、将来イベント種別を分けた専用ページとして実装する。現時点では公演一覧へ混ぜない。
...
- 2026-07-25にログイン機能のローカル実装へ着手。Supabase AuthとNext.js CookieベースSSRを使い、Google／メールアドレス・パスワード、メール確認、パスワード再設定、ログアウトを実装した。
- マイページには、自分の投稿履歴、総合当選率、推しアーティスト、表示名、任意のXアカウント公開設定を置く。メールアドレスは投稿上に公開しない。
